...
- All public *methods* must have a docstring (D102) — check the stack file's ruff ignore list
  before assuming D103 (missing docstring in public *function*) is enforced too: it's ignored in
  this repo's `pyproject.toml`, so bare module-level functions don't require one here even though
  methods do.
