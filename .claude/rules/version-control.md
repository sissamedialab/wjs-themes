...
section, leave no placeholder text. The template is (verbatim from this repo):
...
> **Note for anyone porting rules from another `wjs-*` repo:** this section's header is
> literally `## Issues Items [URL]:` in this repo's committed template — not
> `[repository#issue_number]:` as in some sibling repos (e.g. `wjs-submission-project`), which
> use the `<repository>#<issue-number>` form instead so GitLab auto-links/closes the issue.
> Follow *this* repo's own placeholder (a URL) rather than assuming the `#`-reference
> convention carries over; if you're unsure which behaviour GitLab actually needs here, ask
...
yourself; populate `## Issues Items [URL]:` per this repo's own template (see the
note above); check off `## Required actions:` items that apply and describe them
under `### Actions details`. The MR title and description are always in
**English**. Let the user validate the description and the **target branch**
